  ...
  return (
    ...
    >
      <div className={styles.itemIcon}>
        {icon && iconColor ? <Icon fontSize={"1.5em"} name={icon} color={iconColor} height={height}></Icon> : <></>}
        {imageName ? <Icon fontSize={"1em"} imageName={imageName} height={imageHeight}></Icon> : <></>}
      </div>
      {!imageName ? <span className={styles.title}>{text}</span> : <></>}
      {price ? (
        <div
          style={{
            width: "100%",
